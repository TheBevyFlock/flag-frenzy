...
mod tests {
    ...
    use crate::{config::WorkspaceConfig, intern::FeatureStorage};
    ...
    fn test_filter_dependencies() {
        ...
        storage.insert("foobar".to_string(), &features_map);
        storage.insert("foo".to_string(), &features_map);
        storage.insert("bar".to_string(), &features_map);
        storage.insert("unrelated".to_string(), &features_map);
